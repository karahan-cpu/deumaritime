...
export default function Calculator() {
  ...
  const handleShipInfoSubmit = (data: ShipInfo) => {
    setShipInfo(data);
    setActiveTab(data.isNewBuild ? "eedi" : "eexi");
  };
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4 sm:space-y-6">
            <div className="overflow-x-auto -mx-3 sm:mx-0 px-3 sm:px-0">
              <TabsList className="inline-flex w-auto min-w-full sm:grid sm:grid-cols-3 lg:grid-cols-11 gap-1.5 sm:gap-2 h-auto p-1">
                ...
                {!shipInfo?.isNewBuild && (
                  <TabsTrigger value="eexi" className="gap-1.5 sm:gap-2 text-xs sm:text-sm whitespace-nowrap px-2 sm:px-3" disabled={!shipInfo} data-testid="tab-eexi">
                    <FileText className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
